...
export default class AutoStaticPlugin {
    id = "tor-exit-nodes";
    name = "Tor Exit Nodes";
    description = "Tor exit node IPs geolocated worldwide";
    icon = Shield;
    category = "Security";
    version = "1.0.0";
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for tor-exit-nodes", e);
        }
        if (data && Array.isArray(data.features)) {
            this._entities = data.features.map((f: any, i: number) => {
                const pt = representativePoint(f.geometry);
                return {
                    id: "tor-exit-nodes-" + (f.id ?? i),
                    pluginId: "tor-exit-nodes",
                    latitude: pt.lat,
                    longitude: pt.lon,
                    altitude: pt.alt,
                    timestamp: new Date(),
    ...
    getLayerConfig() {
        return { color: "#a78bfa", clusterEnabled: true, clusterDistance: 50, maxEntities: 7000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(Shield, { color: "#a78bfa" });
        }
