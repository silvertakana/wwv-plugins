...
export default class AutoStaticPlugin {
    id = "dams";
    name = "Dams";
    description = "Dam locations worldwide from OSM";
    icon = Waves;
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for dams", e);
        }
    ...
    getLayerConfig() {
        return { color: "#60a5fa", clusterEnabled: true, clusterDistance: 50, maxEntities: 5000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(Waves, { color: "#60a5fa" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#60a5fa" };
    }
}
