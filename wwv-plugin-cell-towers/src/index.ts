...
export default class AutoStaticPlugin {
    id = "cell-towers";
    name = "Cell Towers";
    description = "Global mobile cell tower coverage - curated subset (OpenCelliD)";
    icon = RadioTower;
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for cell-towers", e);
        }
    ...
    getLayerConfig() { return { color: "#26c281", clusterEnabled: true, clusterDistance: 50, maxEntities: 5000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(RadioTower, { color: "#26c281" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#26c281" };
    }
}
