...
export default class AutoStaticPlugin {
    ...
    description = "Amateur and SatNOGS satellite ground stations worldwide";
    icon = SatelliteDish;
    category = "Space";
    version = "1.0.0";
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for satground-stations", e);
        }
        if (data && Array.isArray(data.features)) {
            this._entities = data.features.map((f: any, i: number) => {
                const pt = representativePoint(f.geometry);
                return {
                    id: "satground-stations-" + (f.id ?? i),
                    pluginId: "satground-stations",
                    latitude: pt.lat,
                    longitude: pt.lon,
                    altitude: pt.alt,
                    timestamp: new Date(),
    ...
    getLayerConfig() {
        return { color: "#22d3ee", clusterEnabled: true, clusterDistance: 50, maxEntities: 7000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(SatelliteDish, { color: "#22d3ee" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#22d3ee" };
    }
}
