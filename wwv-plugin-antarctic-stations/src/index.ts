...
export default class AutoStaticPlugin {
    ...
    description = "Year-round Antarctic research stations (COMNAP)";
    icon = Snowflake;
    category = "science";
    version = "1.0.0";
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for antarctic-stations", e);
        }
        if (data && Array.isArray(data.features)) {
            this._entities = data.features.map((f: any, i: number) => {
                const pt = representativePoint(f.geometry);
                return {
                    id: "antarctic-stations-" + (f.id ?? i),
                    pluginId: "antarctic-stations",
                    latitude: pt.lat,
                    longitude: pt.lon,
                    altitude: pt.alt,
                    timestamp: new Date(),
                    label: f.properties?.name ?? null,
    ...
    getLayerConfig() { return { color: "#5ab0e8", clusterEnabled: true, clusterDistance: 50, maxEntities: 5000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(Snowflake, { color: "#5ab0e8" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#5ab0e8" };
    }
}
