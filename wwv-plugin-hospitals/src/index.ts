...
export default class AutoStaticPlugin {
    id = "hospitals";
    name = "Hospitals";
    description = "Hospital locations worldwide";
    icon = HeartPulse;
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for hospitals", e);
        }
    ...
    getLayerConfig() {
        return { color: "#f43f5e", clusterEnabled: true, clusterDistance: 50, maxEntities: 7000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(HeartPulse, { color: "#f43f5e" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#f43f5e" };
    }
}
