...
export default class AutoStaticPlugin {
    ...
    description = "Oil and gas pipelines and LNG terminals between countries (Global Energy Monitor)";
    icon = Waypoints;
    category = "Energy";
    version = "1.0.0";
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for oil-gas-pipelines", e);
        }
        if (data && Array.isArray(data.features)) {
            this._entities = data.features.map((f: any, i: number) => {
                const pt = representativePoint(f.geometry);
                return {
                    id: "oil-gas-pipelines-" + (f.id ?? i),
                    pluginId: "oil-gas-pipelines",
                    latitude: pt.lat,
                    longitude: pt.lon,
                    altitude: pt.alt,
                    timestamp: new Date(),
    ...
    getLayerConfig() {
        return { color: "#f4592f", clusterEnabled: true, clusterDistance: 50, maxEntities: 5000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(Waypoints, { color: "#f4592f" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#f4592f" };
    }
}
