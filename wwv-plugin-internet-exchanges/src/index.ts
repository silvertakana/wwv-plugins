...
export default class AutoStaticPlugin {
    ...
    description = "Internet Exchange Points and carrier-neutral facilities worldwide";
    icon = Network;
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for internet-exchanges", e);
        }
        if (data && Array.isArray(data.features)) {
            this._entities = data.features.map((f: any, i: number) => {
                const pt = representativePoint(f.geometry);
                return {
                    id: "internet-exchanges-" + (f.id ?? i),
                    pluginId: "internet-exchanges",
                    latitude: pt.lat,
                    longitude: pt.lon,
                    altitude: pt.alt,
                    timestamp: new Date(),
    ...
    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(Network, { color: "#38bdf8" });
        }
