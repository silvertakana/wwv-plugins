 ...
 * internet-exchanges plugin. Exits 0 on PASS, 1 on FAIL.
 *
 * Checks:
 ...
 *   - required properties (name, type) present
 *   - type property is "ixp" or "facility"
...
const REQUIRED_PROPS = ["name", "type"];
const VALID_TYPES = new Set(["ixp", "facility"]);
...
for (const f of features) {
    ...
    if (type != null) {
        if (!VALID_TYPES.has(type)) badType++;
...
console.log(`internet-exchanges data check`);
console.log(`  features:        ${features.length}`);
console.log(`  ixp:             ${typeCounts["ixp"] ?? 0}`);
console.log(`  facility:        ${typeCounts["facility"] ?? 0}`);
console.log(`  bad geometries:  ${badGeom}`);
console.log(`  out-of-range:    ${outOfRange}`);
console.log(`  missing props:   ${missingProps}`);
console.log(`  bad types:       ${badType}`);
...
if (badType > 0) fail(`${badType} feature(s) have invalid type property (expected "ixp" or "facility")`);
