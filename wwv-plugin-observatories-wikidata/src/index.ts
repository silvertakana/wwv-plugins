...
export default class AutoStaticPlugin {
    id = "observatories-wikidata";
    name = "Astronomical Observatories";
    description = "Astronomical observatories and radio telescopes (Wikidata)";
    icon = Telescope;
    category = "space";
    version = "1.0.0";
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for observatories-wikidata", e);
        }
        if (data && Array.isArray(data.features)) {
            this._entities = data.features.map((f: any, i: number) => {
                const pt = representativePoint(f.geometry);
                return {
                    id: "observatories-wikidata-" + (f.id ?? i),
                    pluginId: "observatories-wikidata",
                    latitude: pt.lat,
                    longitude: pt.lon,
                    altitude: pt.alt,
                    timestamp: new Date(),
    ...
    getLayerConfig() { return { color: "#a78bfa", clusterEnabled: true, clusterDistance: 50, maxEntities: 5000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(Telescope, { color: "#a78bfa" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#a78bfa" };
    }
}
