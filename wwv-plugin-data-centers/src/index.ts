...
export default class AutoStaticPlugin {
    id = "data-centers";
    name = "Data Centers";
    description = "Data center locations worldwide";
    icon = Server;
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for data-centers", e);
        }
        if (data && Array.isArray(data.features)) {
            this._entities = data.features.map((f: any, i: number) => {
                const pt = representativePoint(f.geometry);
                return {
                    id: "data-centers-" + (f.id ?? i),
                    pluginId: "data-centers",
                    latitude: pt.lat,
                    longitude: pt.lon,
                    altitude: pt.alt,
                    timestamp: new Date(),
    ...
    getLayerConfig() {
        return { color: "#34d399", clusterEnabled: true, clusterDistance: 50, maxEntities: 7000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(Server, { color: "#34d399" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#34d399" };
    }
}
