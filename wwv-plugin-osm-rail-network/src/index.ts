...
export default class AutoStaticPlugin {
    ...
    description = "Global mainline railway network (Natural Earth)";
    icon = TrainFront;
    category = "transport";
    version = "1.0.0";
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for osm-rail-network", e);
        }
        if (data && Array.isArray(data.features)) {
            this._entities = data.features.map((f: any, i: number) => {
                const pt = representativePoint(f.geometry);
                return {
                    id: "osm-rail-network-" + (f.id ?? i),
                    pluginId: "osm-rail-network",
                    latitude: pt.lat,
                    longitude: pt.lon,
                    altitude: pt.alt,
                    timestamp: new Date(),
    ...
    getLayerConfig() { return { color: "#d97706", clusterEnabled: true, clusterDistance: 50, maxEntities: 5000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(TrainFront, { color: "#d97706" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#d97706" };
    }
}
