...
export default class AutoStaticPlugin {
    id = "power-plants";
    name = "Power Plants";
    description = "Global power plants (thermal, hydro, coal, gas, and more)";
    icon = Zap;
    category = "Energy";
    version = "1.0.0";
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for power-plants", e);
        }
        if (data && Array.isArray(data.features)) {
            this._entities = data.features.map((f: any, i: number) => {
                const pt = representativePoint(f.geometry);
                return {
                    id: "power-plants-" + (f.id ?? i),
                    pluginId: "power-plants",
                    latitude: pt.lat,
                    longitude: pt.lon,
                    altitude: pt.alt,
                    timestamp: new Date(),
    ...
    getLayerConfig() {
        return { color: "#f87171", clusterEnabled: true, clusterDistance: 50, maxEntities: 7000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(Zap, { color: "#f87171" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#f87171" };
    }
}
