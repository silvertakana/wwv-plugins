...
export default class AutoStaticPlugin {
    ...
    description = "Weather station network - active GHCN stations, curated subset (NOAA)";
    icon = CloudSun;
    category = "Weather";
    version = "1.0.0";
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for weather-stations", e);
        }
    ...
    getLayerConfig() { return { color: "#4f8bff", clusterEnabled: true, clusterDistance: 50, maxEntities: 5000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(CloudSun, { color: "#4f8bff" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#4f8bff" };
    }
}
