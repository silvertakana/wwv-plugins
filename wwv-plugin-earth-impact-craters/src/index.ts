...
export default class AutoStaticPlugin {
    id = "earth-impact-craters";
    name = "Earth Impact Craters";
    description = "Confirmed Earth impact crater locations";
    icon = CircleDot;
    category = "science";
    version = "1.0.0";
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for earth-impact-craters", e);
        }
        if (data && Array.isArray(data.features)) {
            this._entities = data.features.map((f: any, i: number) => {
                const pt = representativePoint(f.geometry);
                return {
                    id: "earth-impact-craters-" + (f.id ?? i),
                    pluginId: "earth-impact-craters",
                    latitude: pt.lat,
                    longitude: pt.lon,
                    altitude: pt.alt,
                    timestamp: new Date(),
    ...
    getLayerConfig() { return { color: "#e07b39", clusterEnabled: true, clusterDistance: 50, maxEntities: 5000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(CircleDot, { color: "#e07b39" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#e07b39" };
    }
}
