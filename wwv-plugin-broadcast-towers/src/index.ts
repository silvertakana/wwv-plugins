...
export default class AutoStaticPlugin {
    ...
    description = "US broadcast and communications antenna structures (FCC ASR)";
    icon = RadioTower;
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for broadcast-towers", e);
        }
        if (data && Array.isArray(data.features)) {
            this._entities = data.features.map((f: any, i: number) => {
                const pt = representativePoint(f.geometry);
                return {
                    id: "broadcast-towers-" + (f.id ?? i),
                    pluginId: "broadcast-towers",
                    latitude: pt.lat,
                    longitude: pt.lon,
                    altitude: pt.alt,
                    timestamp: new Date(),
    ...
    getLayerConfig() {
        return { color: "#f59e0b", clusterEnabled: true, clusterDistance: 50, maxEntities: 7000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(RadioTower, { color: "#f59e0b" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#f59e0b" };
    }
}
