...
export default class AutoStaticPlugin {
    ...
    description = "1,209 iron and steel plants - industrial supply chain map (Global Energy Monitor)";
    icon = Factory;
    category = "Industry";
    version = "1.0.0";
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for iron-steel-plants", e);
        }
        if (data && Array.isArray(data.features)) {
            this._entities = data.features.map((f: any, i: number) => {
                const pt = representativePoint(f.geometry);
                return {
                    id: "iron-steel-plants-" + (f.id ?? i),
                    pluginId: "iron-steel-plants",
                    latitude: pt.lat,
                    longitude: pt.lon,
                    altitude: pt.alt,
                    timestamp: new Date(),
    ...
    getLayerConfig() {
        return { color: "#8d6e63", clusterEnabled: true, clusterDistance: 50, maxEntities: 5000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(Factory, { color: "#8d6e63" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#8d6e63" };
    }
}
