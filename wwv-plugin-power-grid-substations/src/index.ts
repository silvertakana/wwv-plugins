...
export default class AutoStaticPlugin {
    ...
    description = "Electrical power grid substations worldwide";
    icon = Cable;
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for power-grid-substations", e);
        }
        if (data && Array.isArray(data.features)) {
            this._entities = data.features.map((f: any, i: number) => {
                const pt = representativePoint(f.geometry);
                return {
                    id: "power-grid-substations-" + (f.id ?? i),
                    pluginId: "power-grid-substations",
                    latitude: pt.lat,
                    longitude: pt.lon,
                    altitude: pt.alt,
                    timestamp: new Date(),
    ...
    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(Cable, { color: "#f59e0b" });
        }
