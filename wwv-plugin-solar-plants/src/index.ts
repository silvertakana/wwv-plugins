...
export default class AutoStaticPlugin {
    id = "solar-plants";
    name = "Solar Plants";
    description = "Utility-scale solar power plants worldwide";
    icon = Sun;
    category = "Energy";
    version = "1.0.0";
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for solar-plants", e);
        }
        if (data && Array.isArray(data.features)) {
            this._entities = data.features.map((f: any, i: number) => {
                const pt = representativePoint(f.geometry);
                return {
                    id: "solar-plants-" + (f.properties?.id ?? i),
                    pluginId: "solar-plants",
                    latitude: pt.lat,
                    longitude: pt.lon,
                    altitude: pt.alt,
                    timestamp: new Date(),
    ...
    getLayerConfig() {
        return { color: COLOR, clusterEnabled: true, clusterDistance: 50, maxEntities: 5000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(Sun, { color: COLOR });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: COLOR };
    }
}
