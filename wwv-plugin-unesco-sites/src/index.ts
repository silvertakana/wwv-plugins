...
export default class AutoStaticPlugin {
    ...
    description = "1,273 UNESCO World Heritage sites (UNESCO, baked snapshot)";
    icon = Landmark;
    category = "Culture";
    version = "1.0.0";
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for unesco-sites", e);
        }
    ...
    getLayerConfig() { return { color: "#e2a54a", clusterEnabled: true, clusterDistance: 50, maxEntities: 5000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(Landmark, { color: "#e2a54a" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#e2a54a" };
    }
}
