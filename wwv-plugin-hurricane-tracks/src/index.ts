...
export default class AutoStaticPlugin {
    ...
    description = "Historical hurricane/cyclone/tropical storm track points worldwide (NOAA IBTrACS, 2000+, category 2+)";
    icon = Tornado;
    category = "Weather";
    version = "1.0.0";
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for hurricane-tracks", e);
        }
        if (data && Array.isArray(data.features)) {
            this._entities = data.features.map((f: any, i: number) => {
                const pt = representativePoint(f.geometry);
                return {
                    id: "hurricane-tracks-" + (f.id ?? i),
                    pluginId: "hurricane-tracks",
                    latitude: pt.lat,
                    longitude: pt.lon,
                    altitude: pt.alt,
                    timestamp: new Date(),
    ...
    getLayerConfig() {
        return { color: "#38bdf8", clusterEnabled: true, clusterDistance: 50, maxEntities: 7000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(Tornado, { color: "#38bdf8" });
        }
