# @worldwideview/wwv-plugin-coral-reefs-global

WorldWideView plugin - Global Coral Reefs

## Description

Global distribution of warm-water coral reefs (UNEP-WCMC). A static, polygon-based globe layer (baked snapshot - not a live feed).
...
```bash
npm install @worldwideview/wwv-plugin-coral-reefs-global
```
...
Fail-closed validator: FeatureCollection parse, Polygon geometry, lon/lat bounds, required name/id props, no duplicate ids, count > 0, file size under limit. Exits 0 on PASS.
