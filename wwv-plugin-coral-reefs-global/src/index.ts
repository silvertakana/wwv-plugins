...
export default class AutoStaticPlugin {
    ...
    name = "Global Coral Reefs";
    description = "Global distribution of warm-water coral reefs (UNEP-WCMC)";
    icon = Waves;
    category = "nature";
    version = "1.0.0";
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for coral-reefs-global", e);
        }
        if (data && Array.isArray(data.features)) {
            this._entities = data.features.map((f: any, i: number) => {
                const pt = representativePoint(f.geometry);
                return {
                    id: "coral-reefs-global-" + (f.id ?? i),
                    pluginId: "coral-reefs-global",
                    latitude: pt.lat,
                    longitude: pt.lon,
                    altitude: pt.alt,
                    timestamp: new Date(),
    ...
    getLayerConfig() { return { color: "#0f9bd8", clusterEnabled: true, clusterDistance: 50, maxEntities: 5000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(Waves, { color: "#0f9bd8" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#0f9bd8" };
    }
}
