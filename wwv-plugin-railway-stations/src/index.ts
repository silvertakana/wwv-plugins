...
export default class AutoStaticPlugin {
    ...
    description = "Railway station locations worldwide";
    icon = Train;
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for railway-stations", e);
        }
        if (data && Array.isArray(data.features)) {
            this._entities = data.features.map((f: any, i: number) => {
                const pt = representativePoint(f.geometry);
                return {
                    id: "railway-stations-" + (f.id ?? i),
                    pluginId: "railway-stations",
                    latitude: pt.lat,
                    longitude: pt.lon,
                    altitude: pt.alt,
                    timestamp: new Date(),
    ...
    getLayerConfig() {
        return { color: "#fbbf24", clusterEnabled: true, clusterDistance: 50, maxEntities: 7000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(Train, { color: "#fbbf24" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#fbbf24" };
    }
}
