...
export default class AutoStaticPlugin {
    id = "shipwrecks";
    name = "Nautical Shipwrecks";
    description = "Shipwrecks and obstructions for nautical history (NOAA data, baked snapshot)";
    icon = Anchor;
    category = "Maritime";
    version = "1.0.0";
    ...
    getLayerConfig() { return { color: "#3f7fb5", clusterEnabled: true, clusterDistance: 50, maxEntities: 5000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(Anchor, { color: "#3f7fb5" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#3f7fb5" };
    }
}
