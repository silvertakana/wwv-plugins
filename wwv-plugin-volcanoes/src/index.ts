...
export default class AutoStaticPlugin {
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for volcanoes", e);
        }
    ...
    getLayerConfig() {
        return { color: "#ff6600", clusterEnabled: true, clusterDistance: 50, maxEntities: 5000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(Mountain, { color: "#ff6600" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#ff6600" };
    }
}
