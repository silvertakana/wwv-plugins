...
export default class AutoStaticPlugin {
    id = "eez-boundaries";
    name = "EEZ Boundaries";
    description = "World Exclusive Economic Zones, territorial seas and high seas from MarineRegions (World EEZ v12)";
    icon = Map;
    category = "maritime";
    version = "1.0.0";
    ...
    async initialize(ctx: any) {
        ...
        try {
            ...
            console.error("Failed to parse data for eez-boundaries", e);
        }
    ...
    getLayerConfig() { return { color: "#3b82f6", clusterEnabled: true, clusterDistance: 50, maxEntities: 5000 };
    }

    renderEntity(_entity: any) {
        if (!this._iconUrl) {
            this._iconUrl = createSvgIconUrl(Map, { color: "#3b82f6" });
        }
        return { type: "billboard" as const, iconUrl: this._iconUrl, color: "#3b82f6" };
    }
}
